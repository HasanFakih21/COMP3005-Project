...
def Q_1(conn, execution_time):
    ...
    query = """SELECT p.player_name, AVG(ps.average_xg) AS avg_xg
FROM players p
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT p.player_name, SUM(ps.shots) AS total_shots
FROM players p
JOIN playerstatistics ps ON p.player_id = ps.player_id
JOIN matches m ON ps.match_id = m.match_id
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT t.team_name, SUM(ts.passes) AS total_passes
FROM teams t
JOIN teamstatistics ts ON t.team_id = ts.team_id
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT t.team_name, SUM(ts.passes) AS passes  
FROM teams t
JOIN teamstatistics ts ON t.team_id = ts.team_id
JOIN matches m ON ts.match_id = m.match_id
WHERE m.season = '2020/2021' AND ts.passes > 0
GROUP BY t.team_name
ORDER BY SUM(ts.passes) DESC;"""
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT p.player_name, SUM(ps.intended_recipient_of_passes) AS total_intended_recipient
FROM players p
JOIN playerstatistics ps ON p.player_id = ps.player_id
JOIN matches m ON ps.match_id = m.match_id
WHERE m.season = '2003/2004' AND ps.intended_recipient_of_passes > 0
GROUP BY p.player_name
ORDER BY total_intended_recipient DESC"""
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT t.team_name, SUM(ts.shots) AS total_shots
FROM teams t
JOIN teamstatistics ts ON t.team_id = ts.team_id
JOIN matches m ON ts.match_id = m.match_id
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT p.player_name, SUM(ps.through_balls) AS total_through_balls
FROM players p
JOIN playerstatistics ps ON p.player_id = ps.player_id
JOIN matches m ON ps.match_id = m.match_id
...
def Q_8(conn, execution_time):
    ...
    query = """SELECT t.team_name, SUM(ts.through_balls) AS total_through_balls
FROM teams t
JOIN teamstatistics ts ON t.team_id = ts.team_id
JOIN matches m ON ts.match_id = m.match_id
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT p.player_name, SUM(ps.successful_dribbles) AS total_successful_dribbles
FROM players p
JOIN playerstatistics ps ON p.player_id = ps.player_id
JOIN matches m ON ps.match_id = m.match_id
...
def Q_10(conn, execution_time):
    ...
    query = """SELECT p.player_name, SUM(ps.dribbled_past) AS total_dribbled_past
FROM players p
JOIN playerstatistics ps ON p.player_id = ps.player_id
JOIN matches m ON ps.match_id = m.match_id
